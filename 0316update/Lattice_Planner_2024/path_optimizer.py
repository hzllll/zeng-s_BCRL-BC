...
class PathOptimizer:
    ...
    def objective(self, p):
        ...
        # global_path_points = self.waypoints  # 获取全局路径点
        # local_path = self.sample_spiral(p)  # 生成局部路径
        # distance_penalty = 0
        # mid_local = int(len(local_path[0]) / 2)
        # dis_min = 999
        # min_id = -1
        # distance_penalty = np.sqrt((local_path[0][mid_local] - global_path_points[int(len(global_path_points)/4)][0]) ** 2 + (
        #         local_path[1][mid_local] - global_path_points[int(len(global_path_points)/4)][1]) ** 2)
        # for j in range(len(global_path_points)):
        #     dis_now = np.sqrt((local_path[0][mid_local] - global_path_points[j][0]) ** 2 + (
        #                 local_path[1][mid_local] - global_path_points[j][1]) ** 2)
        #     if dis_now < dis_min:
        #         dis_min = dis_now
        #         min_id = j
        # if min_id > 0:
        #     distance_penalty = np.sqrt((local_path[0][mid_local] - global_path_points[min_id][0]) ** 2 + (
        #                 local_path[1][mid_local] - global_path_points[min_id][1]) ** 2)
        #
        # min_id_local = [-1] * min(len(local_path[0]),len(local_path[1]))
        # dis_min_local = [999] * min(len(local_path[0]), len(local_path[1]))
        # for j in range(len(global_path_points)):
        #     for i in range(min(len(local_path[0]),len(local_path[1]))):
        #         dis_now = np.sqrt((local_path[0][i] - global_path_points[j][0]) ** 2 + (local_path[1][i] - global_path_points[j][1]) ** 2)
        #         if dis_now < dis_min_local[i]:
        #             dis_min_local[i] = dis_now
        #             min_id_local[i] = j
        # for m in range(len(min_id_local)):
        #     if min_id_local[m] > 0:
        #         distance_penalty = distance_penalty + np.sqrt((local_path[0][m] - global_path_points[min_id_local[m]][0]) ** 2 + (local_path[1][m] - global_path_points[min_id_local[m]][1]) ** 2)
        #
        # for i in range(min(len(local_path[0]),len(local_path[1]),len(global_path_points))):
        #     # for j in range(len(global_path_points)):
        #     diff = np.sqrt((local_path[0][i] - global_path_points[i][0]) ** 2 + (local_path[1][i] - global_path_points[i][1]) ** 2)
        #     distance_penalty = distance_penalty + diff
        # distance_penalty = np.sum(np.sqrt((local_path[0] - global_path_points[:, 0]) ** 2 +
        #                                   (local_path[1] - global_path_points[:, 1]) ** 2))
        #
        # p = [0.0, p[0], p[1], 0.0, p[2]]
        #
        # return self.fbe(p) + 25 * (self.fxf(p) + self.fyf(p)) + 30 * self.ftf(p) + 30 * distance_penalty
