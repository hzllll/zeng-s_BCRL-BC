...
if __name__ == "__main__":
    # 1. 定义所有实验的超参数配置 (根据你的 LSTM 训练配置修改)
    experiments = [
        {"name": "LSTM", "embed_dim": 128, "hidden_dim": 256, "num_layers": 2, "bs": 1024},
    ]
    ...
    checkpoints_dir = "/root/autodl-tmp/BCRL/bc/LSTM_checkpoints" # 请根据实际路径修改

    for exp in experiments:
        print(f"\n{'='*50}")
        print(f"🚀 开始处理实验: {exp['name']}")
            
        # model_pattern = os.path.join(checkpoints_dir, lstm_trajectory_model_0422_1024BSIZE_256HDdim_2layers_150es_CoAnWarmRest_zDATASET.pth)
        ...
        model_filename = "lstm_trajectory_model_0422_1024BSIZE_256HDdim_2layers_150es_CoAnWarmRest_zDATASET.pth"
        ...
        model = LSTMTrajectoryPredictor(
            ...
            num_layers=exp['num_layers'], dropout=0.0
        ).to(DEVICE)
