...
def calc_gradient_penalty(D, expert_data, generated_data):
    alpha = torch.rand(expert_data.size(0), 1)
    alpha = alpha.expand(expert_data.size())
    alpha = alpha.to(device)

    interpolated = alpha * expert_data.data + (1 - alpha) * generated_data.data
...
if __name__ == "__main__":
    ...
    for iteration in range(1, 500):
        ...
        #     exp_obs_batch = expert_obs[batch_indices]
        #     exp_act_batch = expert_act[batch_indices]
        #     obs_batch = obs[batch_indices].view(-1, obs.size(-1))
        #     actions_clip_batch = actions_clip[batch_indices].view(-1, actions.size(-1))
        #     # 计算每个批次的判别器损失
        #     expert_score = D.discriminator(torch.cat((exp_obs_batch, exp_act_batch), dim=1))
        #     generator_score = D.discriminator(torch.cat((obs_batch, actions_clip_batch), dim=1))
        #     expert_loss = nn.functional.binary_cross_entropy(expert_score, torch.zeros_like(expert_score))
        #     generator_loss = nn.functional.binary_cross_entropy(generator_score, torch.ones_like(generator_score))
        #     discriminator_loss = expert_loss + generator_loss + args.gp_lambda * calc_gradient_penalty(D.discriminator, torch.cat((exp_obs_batch, exp_act_batch), dim=1), torch.cat((obs_batch.view(-1, obs.size(-1)), actions_clip_batch.view(-1, actions.size(-1))), dim=1))
        #     # discriminator_loss = expert_loss + generator_loss
        #     # 更新判别器
        #     optimizer_d.zero_grad()
        #     discriminator_loss.backward()
        #     optimizer_d.step()
        #     es += expert_score.sum().item()
        #     gs += generator_score.sum().item()
        # # 输出 expert_score 和 generator_score
        # print(f'expert_score: {es / args.num_steps}, generator_score: {gs / args.num_steps}')
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # bc-gail loss = -log pi(a|s)
                # 从专家数据中随机采样一个迷你批次,计算专家数据的logprob
                _, exp_logprob, _, _ = agent.get_action_and_value(expert_obs[mb_inds], expert_act[mb_inds])
                bc_gail_loss = -exp_logprob.mean()
                ...
                loss = args.gamma_gail * bc_gail_loss + (1 - args.gamma_gail) * pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
        ...
        # 衰减gamma_gail
        args.gamma_gail *= 0.5 ** (1 / args.decay_steps)
        print(f'gamma_gail: {args.gamma_gail}')
