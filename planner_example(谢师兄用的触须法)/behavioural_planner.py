...
class BehaviouralPlanner(object):
    def __init__(self, lookahead, stopsign_fences = None):#  stopsign_fences,
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            closest_len, closest_index = get_closest_index(waypoints[:, :2], ego_state)
...
def get_closest_index(waypoints, ego_state):
    ...
    distance = np.sqrt(np.sum((waypoints - pos[None, :])**2, axis=1))
    closest_index = np.argmin(distance)
    closest_len = distance[closest_index]
