...
EXP_NAME = "Exp-2"  # 每次跑之前改一下这里和下面的参数
EXP_NAME = "Exp-2_v2"

# 模型超参数配置
D_MODEL = 512           # Transformer 的隐藏维度
FFN_DIM = 4 * D_MODEL   # 前馈层维度，经验上 4×d_model 比较常见
num_encoder_layers = 4  # encoder层数 (2 -> 3)
num_decoder_layers = 4  # decoder层数 (2 -> 3)
...
# LEARNING_RATE = 5e-4
LEARNING_RATE = 1e-4  # 或者更保守一点用 5e-5
# LEARNING_RATE = 0.001
# BATCH_SIZE = 128  # 批次大小
BATCH_SIZE = 1024
EPOCHS = 512  # 最大训练轮数
...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4) # 后续保持或略增weight_decay
