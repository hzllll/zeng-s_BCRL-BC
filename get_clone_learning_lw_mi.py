...
# 模型超参数配置
# EMBED_DIM = 32  # 嵌入维度 32
# HIDDEN_DIM = 64  # GRU隐藏层维度 64 
EMBED_DIM = 128  
HIDDEN_DIM = 256  
...
# LEARNING_RATE = 0.001
LR_DECAY = 0.01  # 学习率衰减率（每epoch衰减(1-LR_DECAY)）
# BATCH_SIZE = 128  # 批次大小
BATCH_SIZE = 1024
EPOCHS = 512  # 最大训练轮数
...
# MODEL_SAVE_PATH = fr"/root/autodl-tmp/clone_learning/checkpoints1\gru_trajectory_model_{str(LR_DECAY)}LRD_{str(BATCH_SIZE)}BSIZE_{str(HIDDEN_DIM)}HDdim_0.1rad_3_1_2lay_lwmi.pth"  # 模型保存路径
...
if __name__ == "__main__":
    ...
    # 加权MSE损失（加速度和转角分别加权）
    weights = torch.tensor([ACCELERATION_WEIGHT, STEERING_WEIGHT] * SEQ_LENGTH, dtype=torch.float32).to(device)
