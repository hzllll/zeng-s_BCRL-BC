...
NUM_LAYERS = 2     # LSTM 层数（LSTM 有遗忘门保护，可适当比 vanilla RNN 深）
...
# ======================== 输出路径 ========================
checkpoints_dir = os.path.join(BASE_DIR, "LSTM_checkpoints")
plots_dir       = os.path.join(BASE_DIR, "LSTM_plots")
os.makedirs(checkpoints_dir, exist_ok=True)
os.makedirs(plots_dir, exist_ok=True)

RUN_DATE_TIME = datetime.now().strftime("%m%d")
_tag = (
    f"{RUN_DATE_TIME}_{BATCH_SIZE}BSIZE_{HIDDEN_DIM}HDdim_"
    f"{NUM_LAYERS}layers_{EARLY_STOPPING_PATIENCE}es_CoAnWarmRest_zDATASET"
)
MODEL_SAVE_PATH     = os.path.join(checkpoints_dir, f"lstm_trajectory_model_{_tag}.pth")
LOSS_PLOT_SAVE_PATH = os.path.join(plots_dir,       f"lstm_loss_curve_{_tag}.svg")


# ======================== 模型定义（LSTM 编码器-解码器）========================
class LSTMTrajectoryPredictor(nn.Module):
    """
    以 LSTM（nn.LSTM）为骨干的轨迹预测器。

    与 GRU 版本的核心区别：
      - nn.LSTM 的隐藏状态是元组 (h_n, c_n)，而 nn.GRU 只有 h_n
        h_n: 短期记忆（隐藏状态），shape = (num_layers, B, hidden_dim)
        c_n: 长期记忆（细胞状态），shape = (num_layers, B, hidden_dim)
      - 编码器传给解码器时需同时传递 h_n 和 c_n
      - 参数量比同规模 GRU 约多 1/3（LSTM 4 个门 vs GRU 3 个门）
      - 对长序列建模能力略强于 GRU，因为细胞状态梯度通路更长

    整体结构：
      Embedding → tanh → LSTM Encoder → LSTM Decoder（自回归展开）→ FC Head → Tanh
    """
    def __init__(
        self,
        embed_dim:  int,
        ...
        # --- 输出头（双层 FC + Tanh，与其他版本保持一致）---
        self.head = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, output_dim),
            nn.Tanh(),   # 输出归一化到 [-1, 1]，与标签对齐
        )
    ...
    def forward(self, x):
        ...
        seq_input = torch.tanh(seq_input)   # 与其他版本保持一致

        # 5. LSTM 编码器
        #    ★ 与 GRU 的关键差异：LSTM 返回 (output, (h_n, c_n)) 而非 (output, h_n)
        enc_out, (h_n, c_n) = self.encoder_lstm(seq_input)
        #    enc_out: (B, 9, hidden_dim)
        #    h_n:     (num_layers, B, hidden_dim)  短期记忆
        #    c_n:     (num_layers, B, hidden_dim)  长期记忆（LSTM 独有）

        enc_out = self.dropout_encoder(enc_out)
        h_n     = self.dropout_encoder(h_n)
        c_n     = self.dropout_encoder(c_n)   # ★ 细胞状态也需要 dropout

        # 6. LSTM 解码器（自回归展开，共 seq_length 步）
        #    初始输入：编码器最后一个时间步的输出
        ...
        for t in range(self.seq_length):
            #    ★ 解码器接收 (h_n, c_n) 元组，而非 GRU/RNN 的单一 hidden
            dec_out, (h_n, c_n) = self.decoder_lstm(decoder_input, (h_n, c_n))
...
def preprocess_data(dataset):
    ...
    actions[:, 0:10:2] = np.clip(actions[:, 0:10:2] / 6,    -1, 1)   # 加速度归一化
    actions[:, 1:10:2] = np.clip(actions[:, 1:10:2] / 0.15, -1, 1)   # 转角归一化
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, patience):
    ...
    for epoch in range(epochs):
        scheduler.step()   # epoch 开头更新 lr（与 Transformer 版保持一致）
